...
func New(availableSeats int, timePerHairCut time.Duration) BarberShop {
	shop := BarberShop{availableSeats: availableSeats, timePerHairCut: timePerHairCut}
	shop.clients = make(chan string, shop.availableSeats)
	shop.barbersCount = 0
...
func (shop *BarberShop) cutHair(barber string, client string) {
	...
	time.Sleep(shop.timePerHairCut)
